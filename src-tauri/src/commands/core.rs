...
pub(crate) fn decrypt_with_sender_key_result_core(
    ...
    let cipher = Aes256Gcm::new_from_slice(&material.key_bytes)
...
pub(crate) fn prepare_existing_sender_key_distribution_core(
    conn: &Connection,
    key: &[u8; 32],
    user_id: &str,
    member_id: &str,
    ...
    Ok(PreparedSenderKeyDistribution {
        distribution_message,
        sender_key_version: material.sender_key_version,
    })
}
...
pub(crate) fn consume_sender_key_distribution_for_member_core(
    conn: &Connection,
    key: &[u8; 32],
    user_id: &str,
    sender_member_id: &str,
    ...
    store_sender_key_with_version_core(
        conn,
        key,
        user_id,
        sender_member_id,
...
pub(crate) fn list_sender_key_materials_core(
    conn: &Connection,
    key: &[u8; 32],
    user_id: &str,
    member_ids: &[String],
) -> Result<Vec<SenderKeyMaterial>, String> {
    list_sender_key_materials_inner(conn, key, user_id, member_ids)
}
